...
exports.getLeaderboard = exports.changePassword = exports.updateUserProfile = exports.followUser = exports.searchUsers = exports.updateUserStatus = exports.getAllUsers = exports.dailyCheckIn = exports.getUserProfile = void 0;
...
const getUserProfile = async (req, res) => {
    ...
    try {
        ...
        const following = await server_1.prisma.follow.findUnique({
            where: {
        ...
        const follower = await server_1.prisma.follow.findUnique({
            where: {
                followerId_followingId: {
...
const dailyCheckIn = async (req, res) => {
    ...
    try {
        ...
        const existingTx = await server_1.prisma.coinTransaction.findFirst({
            where: {
                userId,
        ...
        await server_1.prisma.$transaction([
            ...
            server_1.prisma.coinTransaction.create({
                data: {
                    userId,
                    amount: coinAmount,
                    reason: 'Daily Check-in'
...
const followUser = async (req, res) => {
    ...
    try {
        const existingFollow = await server_1.prisma.follow.findUnique({
            where: { followerId_followingId: { followerId: userId, followingId: targetId } }
        });
        if (existingFollow) {
            // Unfollow
            await server_1.prisma.follow.delete({
                where: { followerId_followingId: { followerId: userId, followingId: targetId } }
            });
            ...
            // Follow
            await server_1.prisma.follow.create({
...
const updateUserProfile = async (req, res) => {
    ...
    try {
        ...
        if (username) {
            const userWithUsername = await server_1.prisma.user.findFirst({
                where: {
        ...
        if (email) {
            const userWithEmail = await server_1.prisma.user.findFirst({
                where: {
